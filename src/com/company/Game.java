...
public class Game {
    ...
    public Game(){
        while (true) {
            System.out.println(Player.ANSI_YELLOW + "Hello!, How many rounds do u want to play? 5-30?" + Player.ANSI_RESET);
            try {
                    ...
                    switch (input) {
                        ...
                        case "5":
                            if(!players.get(j).checkMatePossibility()){
                                System.out.println("You don't have the animals/gender required to mate");
    ...
    public boolean tradeWithPlayers(Player p){
        ...
        if (input.equals("2")) {
            if (p.animals.size() <= 0) {
                System.out.println("You don't have any animals. Press any key following by enter to continue");
                scanner.next();
                tradeWithPlayers(p);
            } else {
        ...
        while (true) {
            try {
                ...
                if (players.get(Integer.parseInt(input) - 1).equals(p)) {
                    System.out.println("I can't trade with myself..!");
                    continue;
                }
                if (players.get(Integer.parseInt(input) - 1).animals.size() <= 0) {
                    System.out.println(players.get(Integer.parseInt(input) - 1).name + " don't have any animals yet.");
                } else {
                    break;
                }
            }catch (Exception e){
